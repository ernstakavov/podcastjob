...
export function formatVacancy(v: Vacancy): string {
  ...
  const responsibilities = escapeHtml(v.responsibilities);
  const requirements = escapeHtml(v.requirements);
  const workingConditions = escapeHtml(v.working_conditions);
